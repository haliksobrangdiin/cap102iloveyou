// screens/HistoryScreen.js - Complete with Filter, Swipe to Trash, & Select All
...
const HistoryScreen = ({ navigation }) => {
  // Tabs: 'recent' | 'saved' | 'archived' | 'trash'
  ...
  const handleTrash = (id) => {
    setConfirmConfig({
      ...
      onConfirm: () => {
        ...
        const newData = historyData.map(item => {
          if (item.id === id) {
            return { ...item, isDeleted: true, isArchived: false };
          }
          return item;
        });
        setHistoryData(newData);
  ...
  const handleRestore = (id) => {
    setConfirmConfig({
      ...
      onConfirm: () => {
        ...
        const newData = historyData.map(item => {
          if (item.id === id) {
            return { ...item, isDeleted: false, isArchived: false };
          }
          return item;
        });
        setHistoryData(newData);
  ...
  // Swipeable List Item
  const SwipeableCard = ({ item, children, onTrash }) => {
    ...
    return (
      <View style={styles.swipeWrapper}>
        {/* Trash Icon behind card */}
        <View style={styles.trashBehind}>
          <Ionicons name="trash-outline" size={28} color="#FFFFFF" />
          <Text style={styles.trashBehindText}>Trash</Text>
        </View>
        <Animated.View
  ...
  const renderListItem = ({ item }) => {
    ...
    return (
      <SwipeableCard item={item} onTrash={() => handleTrash(item.id)}>
        <TouchableOpacity 
          style={styles.cardWrapper}
          onPress={() => handleViewDetails(item)}
          activeOpacity={0.7}
        >
          <View style={[styles.historyCard, isDeleted && styles.archivedCard, isSelected && styles.selectedCard]}>
            <View style={styles.cardRow}>
                  ...
                  <View style={styles.badgeRow}>
                    ...
                    {isDeleted && (
                      <View style={styles.archivedBadge}>
                        <Text style={styles.archivedBadgeText}>TRASH</Text>
                      </View>
                    )}
                  </View>
                </View>
              ...
              {/* Trash specific actions OR normal actions */}
    ...
    return (
      <SwipeableCard item={item} onTrash={() => handleTrash(item.id)}>
        <TouchableOpacity 
          ...
          activeOpacity={0.7}
        >
          <View style={[styles.detailedCard, isDeleted && styles.archivedCard, isSelected && styles.selectedCard]}>
            {isSelectAll && (
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.filterOptions}>
                {diseaseTypes.map((type) => (
                  <TouchableOpacity
                    ...
                    onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setSelectedFilter(type); }}
                  >
              ...
              <View style={styles.sortOptions}>
                {[
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setSortBy(option.id); }}
                  >
            ...
            <TouchableOpacity style={styles.resetButton} onPress={() => { setSelectedFilter('All'); setSortBy('newest'); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}>
        ...
        <View style={styles.detailModalOverlay}>
          <View style={styles.detailModalContent}>
            <View style={styles.detailModalHeader}>
              <TouchableOpacity style={styles.detailCloseButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setDetailModalVisible(false); }}>
                <Ionicons name="arrow-back" size={24} color={colors['on-surface']} />
              </TouchableOpacity>
              <Text style={styles.detailModalTitle}>Analysis Details</Text>
              <TouchableOpacity style={styles.detailExportButton} onPress={() => exportReport(selectedItem)} disabled={isExporting}>
                {isExporting ? <ActivityIndicator size="small" color={colors.secondary} /> : <Ionicons name="download-outline" size={24} color={colors.secondary} />}
              </TouchableOpacity>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.detailScrollContent}>
              ...
              <View style={styles.detailStatsRow}>
                ...
                <View style={styles.detailStatDivider} />
                <View style={styles.detailStat}>
                  <Text style={styles.detailStatValue}>{new Date(selectedItem.createdAt).toLocaleDateString()}</Text>
              ...
              <View style={styles.detailSection}>
                <View style={styles.detailSectionHeader}>
                  ...
                  <TouchableOpacity onPress={() => { setDetailModalVisible(false); setTimeout(() => openNotesModal(selectedItem), 300); }}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, styles.notesModalContent]}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setNotesModalVisible(false)} style={styles.modalCloseButton}>
                <Ionicons name="close" size={24} color={colors['on-surface']} />
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.notesActions}>
              <TouchableOpacity style={[styles.notesButton, styles.notesCancelButton]} onPress={() => setNotesModalVisible(false)}>
                <Text style={styles.notesCancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.notesButton, styles.notesSaveButton]} onPress={saveNote}>
    ...
    <Modal visible={confirmModalVisible} transparent animationType="fade" onRequestClose={() => setConfirmModalVisible(false)}>
      <View style={styles.modalOverlay}>
        <View style={styles.savedModalCard}>
          ...
          <TouchableOpacity style={styles.savedModalButton} onPress={confirmConfig.onConfirm} activeOpacity={0.85}>
            <Text style={styles.savedModalButtonText}>{confirmConfig.confirmLabel}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.confirmCancelButton} onPress={() => setConfirmModalVisible(false)} activeOpacity={0.7}>
    ...
    <Modal visible={successModalVisible} transparent animationType="fade" onRequestClose={() => setSuccessModalVisible(false)}>
      <View style={styles.modalOverlay}>
        <View style={styles.savedModalCard}>
          ...
          <TouchableOpacity style={styles.savedModalButton} onPress={() => setSuccessModalVisible(false)} activeOpacity={0.85}>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          ...
          <TouchableOpacity style={styles.headerIconButton} onPress={handleSelectAll}>
            <View style={[styles.checkbox, isSelectAll && styles.checkboxActive]}>
              {isSelectAll && <Ionicons name="checkmark" size={16} color="#FFFFFF" />}
            </View>
          </TouchableOpacity>
      ...
      {/* If in trash and items selected, show bulk actions */}
      {activeTab === 'trash' && selectedIds.length > 0 && (
        <View style={styles.bulkActions}>
          <TouchableOpacity style={styles.bulkButton} onPress={() => {
          ...
          <TouchableOpacity style={styles.bulkButton} onPress={() => {
            selectedIds.forEach(id => handleDeleteForever(id));
            setSelectedIds([]);
            setIsSelectAll(false);
          }}>
            <Ionicons name="trash-outline" size={18} color={colors.error} />
      ...
      <View style={styles.toggleContainer}>
        {['recent', 'saved', 'archived', 'trash'].map((tab) => (
          <TouchableOpacity
            key={tab}
            style={[styles.toggleButton, activeTab === tab && styles.toggleButtonActive]}
            onPress={() => { setActiveTab(tab); setSelectedIds([]); setIsSelectAll(false); }}
            activeOpacity={0.7}
          >
      ...
      {filteredData.length > 0 ? (
        ...
        <View style={styles.emptyContainer}>
          <Ionicons 
            name={activeTab === 'recent' ? 'document-text-outline' : activeTab === 'saved' ? 'bookmark-outline' : activeTab === 'archived' ? 'archive-outline' : 'trash-outline'} 
            size={60} 
            color={colors.secondary} 
          />
          <Text style={styles.emptyText}>
            {activeTab === 'recent' ? 'No active scans' : activeTab === 'saved' ? 'No saved scans' : activeTab === 'archived' ? 'No archived scans' : 'No items in trash'}
          </Text>
          <Text style={styles.emptySubtext}>
            {activeTab === 'recent' 
              ...
              : 'Deleted scans will appear here'}
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  // Swipe Card
  swipeWrapper: { position: 'relative', marginBottom: spacing.sm },
  trashBehind: { position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, backgroundColor: colors.error, borderRadius: rounded.DEFAULT, alignItems: 'flex-end', justifyContent: 'center', paddingRight: 20 },
  trashBehindText: { color: '#FFFFFF', fontSize: 12, fontWeight: '700', marginTop: 4 },
  ...
  detailedActionButtons: { flexDirection: 'row', gap: spacing.xs },
